...
- **快捷键扩充**：`Ctrl+F` 与 `Ctrl+K` 同义（开侧栏并聚焦搜索框）；`Ctrl+O` 打开文件对话框（与顶栏同一函数，按住 repeat 不重复弹出）；`Ctrl+=` / `Ctrl+-` 沿 13 / 14 / 16 / 18 步进正文字号、`Ctrl+0` 复位默认档——与设置页分段选择器同一条「先钉视口」路径，且始终吞键不让渡给 WebView 的整页缩放加速键（导出视图打开时只吞不改：预览分页按当前字号排定）；`F3` / `Shift+F3` 在搜索匹配间前进 / 回退（设置与导出整页视图打开时静默吞键）。设置页快捷键一览同步更新为九行。
- **图片点击查看器**：阅读视图点击正文图片就地放大——宣纸底色（94%）近全屏遮罩而非深色灯箱，滚轮 1–5 倍缩放、放大后指针拖拽平移、双击图片 1↔2 倍切换，`Esc` 或点纸面背底收起（Esc 走 window 捕获段拦截，窄屏不连带关侧栏）。链接包裹的图让位给链接、编辑视图的点击仍归块激活；查看器经 portal 挂 `document.body`，正文 `components` 映射不动。
- **脚注浮笺**（设计定稿 A1）：阅读视图悬停脚注上标 200ms 出小卡——mono 题签「注 n」+ 脚注正文（克隆自文尾 `li`，剥回链与重复 id，不走 innerHTML）；ivory 底 + inset 发丝描边，默认锚点上方、放不下翻下方，指针可移入卡内续读；滚动 / 卡外点按 / `Esc` / 进编辑视图即收。
- **wikilink 笺页卡**（设计定稿 B1）：悬停已解析 `[[wikilink]]` 350ms 出 360px 卡片——笔记题、mono 路径行、剥 frontmatter 后的开头正文（wikilink 语法换显示标签，img 不加载、链接降 span），底缘 36px 渐隐进 ivory；整卡可点，与点链接同一条打开路径。未解析的「纸屑」链永远不出卡（只留原生 tooltip）。内容经新增的 `read_note_preview` 命令读取：白名单由 `resolve_wikilinks` 对当前文档的解析命中同步建立、换文档即清，单篇至多 64KiB 且截断点落在合法 UTF-8 边界。
- **专注模式「留一线」**（设计定稿 C2，`F11` 进出）：顶栏上滑 200ms 隐去、侧栏按进入前的开合态收放、窗口全屏（WebView `setFullscreen`），顶缘留 2px 靛青阅读进度线；指针探入顶缘 8px 感应带顶栏暂驻滑回。`Esc` 亦退出（落在输入控件上或已被消费的 Esc 归它们）；进入时底部浮「F11 · ESC 退出」提示章 2s 淡出。设置页快捷键一览同步更新为十行。
- **侧栏页签 + 库内三面板**：同一枚 `.outline-sidebar` 顶上换成四枚页签（目錄 / 文件 / 檢索 / 反鏈，题头槽位与语汇沿用 `.outline-panel__header` 的位置），只换内容不换壳——「文件」是库内 Markdown 文件树（目录折叠、当前篇高亮并自动展开祖先目录、relPath 筛选退成扁平清单），「檢索」是全库逐行大小写不敏感检索（300ms 防抖、过期响应经请求号守卫丢弃、命中片段按 char 索引切 `mark.search-match`、点命中行打开该篇并把词交给文内检索），「反鏈」列出链到本篇的笔记与至多五行摘录（别名 / 片段 / `![[嵌入]]` 写法都算、代码围栏内不算）。后端三个命令 `list_library` / `search_library` / `find_backlinks` 一律锚定 `AppState.current`——前端不传路径；遍历共享 wikilink 解析那套限深限项规则（库根 = 含 `.obsidian` 的最近祖先目录，否则文档所在目录），重活全部进 `spawn_blocking`。
- **`Ctrl+Shift+F` 全库检索**：开侧栏、切「檢索」页签并聚焦库检索框——判定先于 Ctrl+K/F 别名，设置 / 导出视图里吞键不动作；Ctrl+K / Ctrl+F 随之在聚焦前先切回「目錄」页签。设置页快捷键一览更新为十一行。

### 安全

- **wikilink 解析收窄基准路径**：`resolve_wikilinks` 现在只接受「当前已加载文档」作为 `from_path`（此前前端可传任意路径借解析结果探测文件存在性）；该命令同时负责把解析命中集写入预览白名单，悬停预览的读取面因此不超出本篇笔记实际链接到的文件。
...
- **界面图标动效**：全套线性图标从静态变为会动的——① **描边入场**：图标挂载时按笔顺逐笔描出（`pathLength="1"` 归一化 + `dashoffset` 1→0，`--i` 定次第，实色点改淡入）；② **悬停微动**：历史导航 ‹ › 与侧栏上/下匹配顺向挪移 2px、关闭与清除叉旋 90°、齿轮咬合 45°、最小化内痕沉入底线、最大化框体微胀、文件夹短划滑入并轻抬、导出箭头下探蓄势、搜索镜片随输入框聚焦抬起、笔尖下压且基线重描一划；③ **换章**：笔 ↔ 书双章叠放（旧章旋缩淡出、新章就地描入，每次切换都重播）、复制 → 对勾/叹号描出后自还、点按导出时箭头落穿纸底再自顶归还（`is-firing` 一回合）；④ **常动**：跳转到底部箭头向底线轻点循环（悬停催急）、齿轮按下态（设置视图打开中）9s/圈慢转。全部纯 CSS transition / keyframes 落地，无 JS 库；`prefers-reduced-motion` 下整体关停。设计原型页 `docs/design/icon-motion.html` 入库。
- **导出为 PDF：题目样式选项**——工具条新增「题目 · 原文 / 居中」分段选择器（复用设置页分段语汇）。默认「原文」：题目按文档自身样式排印；「居中」：注入居中题目。正文自带同题 h1 时居中模式用它的原文文本（排版更讲究的那份）作题。

### 修复

- **导出为 PDF：同一题目不再排两次**——正文首块 h1 与文件名派生题归一相同（连字符 / 空白抹平比对）即认出「正文自带头题」：「原文」模式下底稿不再重复注入题目；「居中」模式下注入居中题目并把正文里那枚同款 h1 摘除。
- **导出预览里题目不再右移**——预览纸页中的题目克隆改为 `markdown-body` 的兄弟（与打印底稿同构）；原先塞进它内部，吃两层 32px 横内边距，比正文右移一层、与 PDF 错位。

## [1.10.0] - 2026-09-21

### 维护

- **仓库结构清理**：删掉全部过程产物与无引用素材——历史计划 / 评审 / 设计稿（`docs/superpowers/`）、临时预览页（`docs/preview/`）、已跟踪的临时输出（`outputs/`，本就在 .gitignore 里）、旧版界面截图与无法追溯来源的宣传素材（`assets/`，18MB）、已失效的技能锁文件（`skills-lock.json`）、已全部完成的待办清单（`TODO.md`）、三个空的其他 agent 目录与 pi 运行时的本地联接（`.pi/` 下 21 个技能联接，只删联接、全局库完好）。接口文档同步收紧：`AGENTS.md` 新增「仓库结构」一节，写明各目录职责与「不新建目录」的要求，硬编码测试计数撤下，并修掉 `scripts/verify-widget-template.mjs`、`obsidian.md`、`extensions/mdlog/README.md`、`vite.config.ts` 里指向已删路径的引用；`docs/agents/tooling.md` 的宣传品一节随之删除。

### 新增

- **导出为 PDF（替换打印，方案「纸张舞台」）**：`Ctrl+P` 与顶栏新导出按钮（齿轮左侧，按下态与齿轮同款）打开整页替换正文区的导出视图——A4 纸页在暖砂舞台上铺开、随窗口缩放，底部一条浮动工具条（规格小字 `A4 · 边距 20/22mm · 宣纸` + 文件名输入框 + 导出按钮），`Esc` / 「‹ 返回阅读」退出，阅读位置进出交接与设置视图同一条管线。页面规格是**模板常量而非选项**，逐字对齐上游 kami 的 WeasyPrint 模板（tw93/kami）：A4 纵向、边距 20mm/22mm、宣纸底色、首页页眉页脚留白、第 2 页起页码右上 + 页脚居中「文档题 · 素笺」。预览分页按行摹 Chromium：段落按行盒拆分（孤行/寡行 = 2）、标题随块换页、页首边距截断，预览 / 测量 / 打印底稿共用同一份消毒后的正文 HTML（界面件与搜索高亮不进打印件）。页眉页脚走 CSS `@page` 边盒（运行时注入，文档题是运行时值），纸面底色刷在 `@page` 上（body 背景在分页时不传播满页边区——真机像素取证修掉的白边）。落盘由后端 `export_pdf` 命令对主窗口 WebView2 直接调 CDP `Page.printToPDF` 完成（同一页面上演，不另起隐藏 webview），保存位置走系统对话框。真机验收：8 页样章页界逐页比对、PPM 逐像素验底色与页眉页脚。

### 移除

- **系统打印对话框**：`Ctrl+P` 不再调 `window.print()`。两段 `@media print` 打印样式继续服役——导出件的排版正是这套规则（打印底稿 `.export-sheet` 屏幕态隐藏、打印独占纸面）。

## [1.9.0] - 2026-09-21

### 新增

- **阅读设置（顶栏齿轮 → 设置页，2026-09-20 第二批定型）**：顶栏左侧按钮簇加一枚齿轮幽灵按钮（28×28，`aria-label="阅读设置"`），点开是替换正文区的**设置页**（早期实现是 236px 顶栏弹层，第二批退役，见「移除」）——「阅读」一节三行分段选择器：正文字号 13 / 14 / 16 / 18、栏宽 720 / 800 / 960、行高 1.5 / 1.55 / 1.7，其下「恢复默认」+ 试笔样张。设置写进与 `outlineWidth` 同一个 settings Store（key `readerSettings`，逐字段校验、非法值回退默认），生效方式是在 `documentElement` 上覆写 `--reader-font-size` / `--reader-column-width` / `--reader-line-height`，`kami.css` 的正文、表格与文档标题消费这三个变量并带默认值回退——变量挂在根元素、与文档无关，热重载与换文档后继续生效。标题字号阶梯（30 / 21 / 17）与行内 code 的 12px 不随设置缩放（设计定稿）。栏宽 / 字号改动会引起整篇重排，故与侧栏拖宽同路径：改值前先走 `beginWidthTransition()` 钉住视口。退出走 `Escape` 或「‹ 返回阅读」；进入时先把阅读位置取下、共用滚动容器归零，退出时按既有落位管线放回原处（窄屏 `Escape` 只退设置视图，不连带关侧栏）。
...
- **打印样式（两段 `@media print`）**：打印只做三件事——隐藏界面件（顶栏 / 侧栏 / 拖宽手柄 / 纱罩 / 自定义滚动条 / 跳底按钮 / 印章 / 编辑提示条 / 记录中的呼吸小章；**设置视图不在此列**——它打开时正文整块不在 DOM，藏掉只会印出一张白纸，弹层时代才需要藏它）、放开版心（屏幕态把正文关在「`100vh` + `overflow:hidden`」的壳里，不放开就只印得出第一屏；列宽放开到 100%、侧栏位移与 42px 顶距显式归零）、分页保护（代码块 / 表格 / 引用 / 图片不跨页，标题不留在页脚）。屏幕态规则一字不动。位置约束由 `kami.css.test.ts` 锁死：主段不含 `.mdlog-widget` 字样且排在首个该选择器之前，含该字样的那一段在文件末尾（隐 chrome 题头栏、恢复 `--parked` 停帧 iframe，以及屏幕态规则落在主段之后的 `.mdlog-live`——记录中的呼吸小章，原因与实测见下面「修复」一节）。正文字号沿用 `--reader-font-size`（用户的阅读设置就是他的选择），不设 `@page`。`Ctrl+P` 绑定 `window.print()`（`typeof window.print === "function"` 守卫：拿不到实现时既不动作也不吞键；`Ctrl+Shift+P` 不归它）。**真机未验证**：WebView2 的打印对话框是否真的弹出仍属未测项（现有证据只有微软 WebView2 打印文档 + headless Chrome 的 `printToPDF` 引擎级验证），故 README 暂不写这条快捷键。
...
- **设置页（替换正文区的整页视图，2026-09-20 第二批）**：正文区整块换成四节内容栏（`SettingsView`，640px 居中，顶行「‹ 返回阅读」+ mono 小字「ESC · 改动即存」）——**阅读**（三行分段选择器 + 「样张」试笔区；样张消费与正文同一组 `--reader-*` 变量，接住弹层退役后失去的「边调边看」）、**界面**（「启动时展开侧栏」两态，出厂关）、**更新**（「启动时自动检查更新」两态，出厂开；当前版本取 `getVersion()`，拿不到显示「—」不编版本号；「立即检查」无论开关都查、结果一律回执）、**关于与数据**（当前文档完整路径，无文档显示「未打开文件」；清空最近打开列表；六条快捷键一览）。侧栏复用同一枚 `.outline-sidebar`，内容换成 `SettingsNav`（题头「設定」+ 搜索框 + 四个分节，类名全用 `.outline-panel*` / `.outline-search*`），宽度、开合、拖宽手柄、窄屏浮层与默认关全部自动跟随；`Ctrl+K` 聚焦同一枚搜索框，点条目缓动滚到对应分节（设置页与正文共用同一滚动容器）。顶栏齿轮呈按下态（warm-sand 底 + brand 字色）标示设置视图打开中。新 store key `sidebarOpenOnLaunch`（出厂关）与 `autoCheckUpdates`（出厂开），与 `outlineWidth` 同一个 settings Store，读盘失败一律回退出厂值。

### 更改

- **UI 修复批量（2026-09-20）**：`Ctrl+B` 在**所有宽度**下切换侧栏开关（侧栏已开且搜索框聚焦时也能关，`Ctrl+K` 只保焦不动作），开关照旧先走 `beginWidthTransition()`；顶栏大纲与打开文件按钮补 `title`（「切换大纲（Ctrl+B）」/「打开文件」）；`ErrorState` 加「重新打开」实色按钮（触发打开文件对话框），空态与错误页共用 `.button.button-primary`（warm-sand 底 + 发丝内描边 + 6px 圆角，高度写死 32px 让两个页面逐像素同高）与 `.empty-eyebrow`；搜索框有查询时右侧出清除 × 按钮（点击清空并保焦），查询非空且 0 匹配时计数位显示「无匹配」、计数容器带 `aria-live="polite"`；窄屏不渲染拖宽手柄（正文不位移，拖了无意义）；`.outline-scrim` 的 z-index 抬到 950（高于自定义滚动条与印章，浮层时侧栏外不可交互）；`.document-scroll` 补克制的 `:focus-visible` 内描边；mdlog 记录中时顶栏齿轮右侧加一枚「记录中」小章（mono 10px、brand 字色），让记录状态不只出现在文档尾部。
- **顶栏纯工具栏化（2026-09-20 第二批）**：路径退出顶栏——`.top-bar__path` 与 `.top-bar__meta` 删除，中列只剩拖动热区 `.top-bar__spacer`；左簇重排为 大纲切换 → 后退 ‹ → 前进 › → 编辑 → 打开文件 → 分隔线 → 设置齿轮 →「记录中」小章（小章从路径右侧挪到齿轮右侧）。完整路径的归宿只有两处：正文 `h1.document-title` 的 `title` tooltip（`state.document.path`）与设置页「关于与数据 · 当前文档」。全部 aria-label / title 文案逐字未动。
- **任务勾选定稿「钤印 + 划线」（2026-09-20 第二批）**：阅读视图的任务复选框自绘——15×15、1px 发丝描边、2px 圆角、透明底、`padding: 0`（不归零会被通用控件规则的 `6px 10px` 顶到 ~22px 宽）；hover 框线转靛青、焦点仍是 1px outline；已勾填 `--tag-bg` 实色 + 靛青 data-URI 对勾（input 是 void 元素塞不进子节点），框线转 brand。已勾项文字灰化（`--stone`）+ 1px 删除线：GFM 产物里文字是复选框的兄弟**文本节点**，定位落在 `<li>` 上用 `:has()` 认「本级自己的复选框已勾」，紧列表 / 松散列表各一条单选择器规则（松散列表的 `<p>` 还要单独补色，它自己的 `color` 会盖掉继承）。全部规则带 `:not(:disabled)`，编辑视图（复选框恒 disabled）与原始 HTML 写死的 disabled 一条都命中不了。打印下已勾文字回到正文色（纸上没有 hover 语境，浅灰只丢对比度；删除线保留）+ `print-color-adjust: exact`（对勾是背景 SVG，关掉「打印背景图形」时框会空成未勾）。**父项已勾时其下嵌套子项视同完成**（删除线与灰化从 `<li>` 传播到块级后代）；反向不成立（`:has()` 用直接子级限定）。
...
- **打印覆写曾对任务勾选整体失效（2026-09-21，第二批；只在构建产物里失效）**：屏幕态那条划线规则原是**一条逗号列表**，构建期 minifier 把整条列表包进 `:is(A, B)`，而 `:is()` 取参数里最高的特异度——紧列表 (0,5,1) 被松散列表 (0,5,2) 抬了一档，打印段里按源码特异度对齐的三条覆写全压不住（headless Chrome 打印媒体实测：修前紧列表 / 松散列表 / 松散列表 `<p>` 三条都仍是灰字）。修法：屏幕态两种形状拆成两条**单选择器**规则（即使被包进 `:is()` 特异度也逐字不变），打印段三条覆写与屏幕态**逐字同选择器**靠来源序取胜。教训：凡打印覆写要压过屏幕态规则，屏幕态规则不能写成逗号列表——源码与 `npm test` 都看不出，必须拿产物验。
- **自绘复选框实测 21.33×15px（2026-09-21，第二批）**：`.markdown-body input` 的通用控件规则给了 `padding: 6px 10px`，`appearance: none` 之后没人再压住它（原生 `appearance: auto` 时 Blink 强制复选框 `padding: 0`，所以以前不显形），border-box 下 `width: 15px` 被 padding 顶宽。修法：自绘基座显式 `padding: 0`。
- **窄屏 `Escape` 一次关两层 / 进设置视图不复位滚动位置（2026-09-21，第二批）**：前者是关侧栏那条监听的守卫里读了 `isSettingsOpen` 却没把它放进依赖表（陈旧闭包恒为 `false`，窄屏 + 先开侧栏 + 后开设置时一次按键关掉两层），修法是补依赖；后者是 `openSettings` 只取下阅读位置、没动共用滚动容器（正文退出 DOM 后内容变矮，浏览器把旧 `scrollTop` 钳到设置页的最大值，从长文档中部进来会落在设置页中段），修法是在**取位置之后**把容器 `scrollTop` 归零。两条都先复现、后修，各配一条撤掉修复即失败的用例。
- **窄屏（≤720px）顶栏按钮溢出底边横线（2026-09-21，第三批）**：`@media (max-width: 720px)` 里有一条初始提交遗留的 `.top-bar { padding: 12px 18px }`——顶栏是 border-box 定高 46px，竖向 padding 把内容盒压到 22px，而 auto 网格行仍按内容（46px 窗口控件）撑高、从内容盒顶端起排，整行溢出顶栏底边（headless Chrome 实测 520px 宽：按钮下沿 49px、网格行下沿 58px，栏底只有 46px）。修法：删除该条覆写，顶栏全宽度共用基准 `padding: 0 0 0 16px`；`kami.css.test.ts` 补「媒体查询内不得再出现 `.top-bar` 规则」断言钉住。
...
- **文档收口（第二批）**：`AGENTS.md` 红线 15 改写为「顶栏不显示文件名与路径」，红线速查顺接补 19（minifier 的 `:is()` 抬高特异度 ⇒ 屏幕态规则不能写成逗号列表）与 20（设置视图只换侧栏内容），关键路径登记 `SettingsView` / `SettingsNav` / `appPreferences` / `updater`，测试计数同步为 **48 个测试文件 / 969 用例**；README 的阅读设置改写为设置页四节、勾选写回补钤印与划线、Usage 第 7 条改写；`docs/agents/rendering.md` 新增「设置视图」一节（正文退出 DOM 的代价、阅读位置交接、视图边界、打印不隐藏），补勾选长相 / `:has()` 引擎前提 / 嵌套传播三条，打印一节补「设置视图不在隐藏清单」与 `:is()` 教训；`docs/agents/tooling.md` 宣传素材一节订正为「顶栏不再显示路径」、更新检查一节改写为 `src/lib/updater.ts` + `autoCheckUpdates`、入口 chunk 尺寸历史补 192.90KB / gzip 54.69KB（CSS 34.74KB）；`docs/agents/obsidian.md` 的顶栏不显示文件名一条订正为不显示文件名与路径；设计定稿档案 `docs/preview/topbar-settings-proposal.html` 与本批计划 `docs/superpowers/plans/2026-09-20-settings-page-topbar.md` 入库。
