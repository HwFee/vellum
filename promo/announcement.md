...
- `Vellum_1.7.0_x64-setup.exe` —— 单用户轻量安装

安装程序会注册 `.md` / `.markdown` 文件关联。Windows 10 1809+ / 11，x64，需要 WebView2。
...
- **一行版（≤80 字）**：给 Markdown 一张纸。暖纸底色、今楷正文、一笔靛青的 Windows 阅读器，
  约 21 MB，开源离线。长文大纲、全文检索、块级就地编辑，还有给 agent 写文档用的现场日志。
...
- **技术向（给做 Tauri / 前端性能的老哥）**：
  Tauri 2 + React 19 的 Markdown 阅读器，约 21 MB 安装包。性能上做了几件不太常见的事：
...
- 「约 21 MB 安装包」指 NSIS 单用户安装包，别写成「约 21 MB 程序」。
