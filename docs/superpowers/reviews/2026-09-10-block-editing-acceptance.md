...
| 交付面 | 内容 |
|---|---|
| 块单元划分 | `src/lib/editUnits.ts`（纯函数）：顶层节点各成一块 + `list` 下钻到 `listItem` + `blockquote` / `footnoteDefinition` 下钻一层（终审 F37 补入脚注）；区间排序、互不重叠并对重叠做归一化 |
| 结构性只读 | 块级 HTML（含 mdlog 头注释）与 `vellum-widget` 围栏**无编辑入口**，点击只给提示（含嵌套在引用/列表/脚注内的情形）；行内 HTML 仍是块内可编辑文本 |
...
| 6 | 提交前记下同一文档中未改动块下方 widget iframe 的 DOM 节点（DevTools 选中），提交后再看 | 未改动块**上方**内容不变时，该 iframe 节点未重建（`=== ` 同一节点）；改动块下方若出现重建属已知代价（§8 第 3 条） |
...
| 9 | 提交耗时实测（用于校准 800ms 阈值）：在散文 101KB / 301KB / 1MB 与 widget 重度 512KB 文档上各提交一次，用 DevTools Performance 或临时 `performance.now()` 计时 | 记录实测毫秒数；若真机普遍远低于 800ms，应下调 `heavyCommitMs`（终审修复波后已接到界面：超阈值时编辑视图内常驻「本文档较大」轻提示，见 §11） |
...
| 14 | 编辑视图 + 有活动块时建立 mdlog 记录（自动化的边沿场景） | 提示一次「记录已开始，编辑已取消」，且不重复刷提示；无活动块时静默退出编辑视图（当前无提示，登记项见 §8 第 8 条） |
...
| 2 | **`heavyDoc` 提示本会话内粘性不回退**（裁定 F26） | 观测到一次超阈值提交后标记不再复位（文档规模属性而非瞬时值）。界面接线已在终审修复波补齐（F40：编辑视图内常驻「本文档较大」轻提示，见 §11），阈值仍待真机校准（手检 #9） |
...
| 10 | **进/出编辑视图会重建全篇 widget iframe**（终审 I2 / 裁定 F42，登记不改） | `.vellum-unit-wrap` 只在 `editable` 为真时由插件插入 ⇒ React 元素层级变化 ⇒ 每次 `Ctrl+E` 进出编辑视图，全篇 widget iframe 销毁重建（重新加载 + `--ready` 淡入闪烁）。与 §8 第 3 条（提交时改动块上方的重建）是两个不同场景。**下一版优化**：把 `data-vellum-unit` / `data-vellum-locked` 像 `h1/h2/h3` 那样交给 `components.pre` 显式还原，只对数学块保留包裹层（katex 会替换掉 `pre` 的属性）。本次不改：要动承担载荷的渲染管线与 T2/T3/T7 三方契约，风险大于它消除的闪烁。手检 #6 只能验证提交路径，本条需手检时对比进出编辑视图前后的 iframe 节点 |
| 11 | **非段落块的行级落点近似精度更低**（终审 T7-I2 / 裁定 F43） | 光标落点本就是「块内纵向比率 → 源码行首」的近似（§8 第 5 条）；标题/列表项/引用在编辑态的字号与行高与阅读态不同（14px 等宽 vs 30/21/17px），因此这些块的行级落点偏差更大。高度下限由 `Math.max(scrollHeight, lockedHeight)` 兜住（F18），不会出现高度跳变，真实影响只是「编辑态字形比原块小」+ 单子块引用容器多 5px 左缩进差。手检 #2/#11 判观感 |
...
2. 畸形围栏单元 `end` 越界（F17）：登记不修。
3. 手检 #12 若判定链接双触发不可接受，需补一行「编辑视图下 `a` 只激活不打开」。
4. 关窗处理器异常分支的防御性 `try/catch`（§8 第 9 条）：待下一轮。
5. 进/出编辑视图重建全篇 widget iframe（§8 第 10 条，裁定 F42）：下一版按 `components.pre` 方案优化。

## 11. 终审修复波登记（2026-09-10，裁定 F37–F43）

全分支终审（`.superpowers/sdd/2026-09-10-vellum-block-editing/final-branch-review.md`）判定「需先修复再合并」；终审修复波（裁定 F37–F43）落地后，本报告下列条目已被修正：

| 裁定 | 内容 | 落地位置与验证 |
|---|---|---|
| F37 | `footnoteDefinition` 作可下钻容器（与 `blockquote` 同列），脚注正文逐块可编辑、其中的 HTML/widget 子块各自锁定；回归改为「容器 × 锁定块」遍历式断言 | `editUnits.ts`（`BLOCK_CONTAINERS` + `collectUnits.drill`）；`editUnits.test.ts` 新增 14 条遍历用例 + 脚注下钻用例，C1 的三条反例现均为 `editable:false` 且 reason 正确 |
| F38 | `Ctrl+S` 双通道去重（A：全局处理器 `event.defaultPrevented` 早退；B：hook 的 `committingRef` 在途闸门） | `App.tsx` / `useDocumentEditor.ts`；`App.test.tsx` 用例把 `Ctrl+S` 打在编辑框上且草稿改变块结构，断言 `save_document` 恰好一次（修复前实测 2 次） |
| F39 | 切换文档（非同路径）时 `resetSession()` 清空编辑会话 | `useDocumentEditor.ts` 暴露 `resetSession`，`App.loadPath` 在确认切换后调用；`App.test.tsx` 用「落盘失败 + 切文档」复现路径断言不再残留旧草稿 |
| F40 | `heavyDoc` 接到界面：编辑视图内常驻轻提示 | `App.tsx` 渲染 `.editor-hint`（`viewMode === 'editing' && heavyDoc`），样式在 `kami.css` 编辑态区段；`App.test.tsx`（注入 `heavyCommitMs: 0`）+ `kami.css.test.ts` 各一条 |
| F41 | 覆盖层位置的 DOM 层守卫 | `App.test.tsx` 断言 `.document-scroll__content--editing > textarea.block-editor__input` 即覆盖层本身 |
...
### 9.7 真机缺陷（严重）：点第一块却整篇滚到底 —— 已修

- **报告**：用户手测「文件有很多块时，准备修改第一块会**自动滑到底部**」。
- **CDP 复现**（widget 重度长文档）：点第一块 → `scrollTop 0 → 1639`（= 文档底部），滚动日志显示滚动发生时 `document.activeElement` 正是 `.block-editor__input`
  ⇒ **是 `focus()` 把页面滚下去的**：激活瞬间覆盖层尚未拿到内联 `top/left`，停在内容末尾的**静态位置**，浏览器为把焦点元素滚入视野一路滚到底，随后内联定位才生效。纯段落文档因时序侥幸未复现。
- **连带效应**：跳到底后阅读位置记忆把「底部」存入 → **该文件下次打开也在底部**（探针中设 `scrollTop=0` 会被立刻拉回）。
- **修复**（`0853d6f`，两处独立防护）：
  1. `focus({ preventScroll: true })` —— 聚焦不再拖动页面；
  2. **盒未就绪（解析失败 / 宿主缺失）时不渲染覆盖层**，而不是渲染在静态位置等聚焦。为此把「定位与占位」（不依赖覆盖层）与「覆盖层度量」（`box` 就绪后跑，并在量高前按块实测对齐行高）拆成两个互不依赖的 effect —— 若仍让定位 effect 要求 `textareaRef.current` 存在，会与「未定位不渲染」互相等待而死锁（该死锁在实现中被 16 条既有用例全红拦下）。
- **验证**（打包版 CDP）：点第一块 `scrollTop` 保持 0；覆盖层 `rectTop=156` 落在视口内；输入并 `Esc` 提交后仍在顶部且磁盘精确写入；中段点击 900 → 900 零漂移。`npm test` 431 用例全绿（新增 `preventScroll` 防回归用例）。
