...
（v3.1 修订：不再写 `# Pi 对话记录` 可见标题——书札版式由宿主承担，文件头只有隐形指纹。）
...
助手消息模板（标准，v3.1 起说话人标签带 `mdlog-who` class 供书札版式渲染）：

```markdown
<p class="mdlog-who"><strong>Pi</strong> · 14:32</p>
...
1. **文件头**：首行写入 `<!-- mdlog:v1 s=<sessionId> -->\n\n`（v3.1 起不含任何可见标题）。注释后必须保留一个空行，与正文区块分隔。
...
### v3.1 补记（书札版式与复审第三轮，2026-09-05）

| 项 | 变更 | 理由 |
|---|---|---|
| §3.4 文件头 | 不再写 `# Pi 对话记录` 可见标题，首行只有隐形指纹 | 用户明示空文档不加标题；版式由宿主书札样式承担 |
| §3.4 助手标签 | `**Pi** · HH:MM` → `<p class="mdlog-who"><strong>Pi</strong> · HH:MM</p>` | 纯 Markdown 段落无法被 CSS 无歧义选中（正文会以粗体起首误中）；sanitize 通配 className 本就放行 |
| §3.4 碎片合并 | 同角色连续消息合并为一个回合块：批内拼装 + 跨批次与文件尾块合并重写（`endsWith` 校验，外部编辑降级为独立追加）；合并重写走 tmp+rename 原子替换 + EPERM 微重试 | 流式多 message_end 事件不再产生重复说话人标签与多条 `---`；重写原子性杜绝并发读者读到 0 字节窗 |
| §3.6 断开语义 | `disconnect` 等待 `destroy({discard})` 完成；写链入口与重试退避均有 `isDisposed` 门禁 | 断开后不再有迟到写盘/顺序颠倒 |
| §3.3 重连 | `connectToFile` 统一校验 `.md/.markdown` 扩展名（手动与自动同闸） | 杜绝指纹头写进 .txt/.bat |
| 宿主版式 | 受信 mdlog 文档挂 `.markdown-body--mdlog` 作用域：D 书札卷轴（章点标签、无 `---` 可见分隔、widget 去 chrome 双细线框）；widget 与正文同栈今楷（字体安装于主机系统） | 用户评审选定 D 变体；沙箱读不到宿主打包字体，系统字体安装是最简穿透方案 |
