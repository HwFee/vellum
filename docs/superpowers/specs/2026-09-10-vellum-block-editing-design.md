...
### 14.1 §6.4 「外部变更且框内有改动」二选一横幅 → 中断路径（已交付 `e82ba24`；后续轮次有修改）
...
### 14.9 终审修复波（裁定 F37–F43，2026-09-10）对本文口径的三处修正

全分支终审判定「需先修复再合并」，修复波后下列正文承诺的实现口径发生变化（裁定优先）：

1. **D2 单元粒度扩展**：`footnoteDefinition` 也作为可下钻容器（与 `blockquote` 同列），其直接块级子节点各成一块 —— 脚注正文因此可逐块编辑。原口径只有 `list` / `blockquote` 两类容器，导致脚注定义内嵌的块级 HTML / `vellum-widget` 被整块当成可编辑块（C1）。注意：下钻仍然只做一层，与 D2 的「只下钻一层」一致。
2. **D4 结构性只读的覆盖范围**：锁定容器的判据由「`list` / `listItem` / `blockquote`」扩展为**全部块级容器**（加入 `footnoteDefinition`，含嵌套在引用/列表里的脚注），回归断言改为「容器 × 锁定块」的遍历式清单，不再逐容器补例子。
3. **D5 软提示可见 + `Ctrl+S` 单通道**：`heavyDoc` 已接到界面（编辑视图内常驻「本文档较大」轻提示），D5 正文的「挂出软提示」可看作已成立；`Ctrl+S` 的双通道提交已去重（全局处理器的 `event.defaultPrevented` 早退 + 会话状态机内的在途提交闸门），D5 的「`Ctrl+S` 等价于提交当前块」仍是字面事实（一次按键 ⇒ 一次提交）。
