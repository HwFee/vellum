...
- **Primary 靛青（#1B365D）：** 唯一的彩色。承担所有交互指向——链接、激活态（大纲当前条目、分段选择器选中项、顶栏按钮按下态）、焦点描边、搜索高亮、任务勾选的钤印方框与「记录中」状态点/印章描边。取自传统青花/印章蓝的沉稳色相，绝不用于大面积填充。页面占比 ≤5%，多余的强调退回字重/字号解决。
...
- 正文 14px / 1.55 行高，letter-spacing 0.4px（CSS 源码中为 0.3pt）——中文排印的舒展感来自字距而非字号。字号与行高是用户可调项（13 / 14 / 16 / 18、1.5 / 1.55 / 1.7 三档，经 `--reader-font-size` / `--reader-line-height` 覆写，Ctrl+= / Ctrl+- / Ctrl+0 快捷键步进与复位）；标题字号阶梯不随它缩放。
...
- 正文列宽 `min(var(--reader-column-width), 100%)` 居中——栏宽是阅读设置之一，720 / 800 / 960px 三档（默认 800）；上下留白 70px / 40px（首行有文档标题时顶部收成 42px），页面 padding 40px 32px（窄屏 24px 16px）。
- 顶栏固定 46px，三列网格（标题区 / 弹性 / 操作区），整栏可拖拽（Tauri 无框窗口）。
- 大纲侧栏默认 240px 宽 + 20px gutter，右缘手柄可拖拽调宽 200–320px（双击复位）；≥720px 时正文向右让位「侧栏宽 + gutter」一整条（默认 260px），<720px 时变为浮层加 15% 墨色纱罩（scrim）。
...
- **分段选择器（segments / seg）：** 设置页「阅读」三件套与导出视图题目选项共用的离散选项控件——透明底 serif 12px 小格（≥34px 宽、4px 圆角、stone 字色），悬停填 warm-sand；选中项不加重量感，只给 brand-tint 淡底 + 靛青字，焦点仍是 2px 靛青描边。
- **导出视图「纸张舞台」（export-view）：** 整页替换正文区——暖砂舞台底上铺开 A4 纸页（794×1123 绢面、发丝描边、8% 黑的一丝投影），页码右上、页脚居中「文档题 · 素笺」、首页留白；选项收进底部一条 sticky 浮动工具条。预览与打印底稿共用同一份消毒 HTML，所见即所得。
- **任务勾选「钤印 + 划线」：** 复选框自绘为 15px 发丝方框（1px hairline、2px 圆角、透明底），悬停框线转靛青；已勾填 tag-bg 实色 + 靛青对勾、框线转 brand，同项文字灰化（stone）加 1px 删除线，父项已勾时嵌套子项视同完成。编辑视图与作者写死的 disabled 一律不命中（`:not(:disabled)` 限定）。
- **图标动效（icon-draw / hover 微动 / 换章）：** 线性图标挂载时按笔顺逐笔描出（`pathLength` 归一 + dashoffset 1→0），悬停各微动 1–2px/几度（叉旋 90°、齿轮咬合 45°、箭头顺向挪移），编辑/复制这类双态图标在原地换章（旧章旋缩淡出、新章描入）；「跳到底部」箭头与设置按下态齿轮有常动循环。纯 CSS 落地，`prefers-reduced-motion` 下全部关停。
- **图片查看器（image-viewer）：** 点击正文图片就地放大——宣纸 94% 底色的近全屏遮罩盖在顶栏之下（纸上读图，不是深色灯箱），图片发丝描边 + 6px 圆角；滚轮 1–5 倍缩放（拖拽平移时关 transform 过渡）、双击 1↔2 倍切换，Esc / 点背底收起。链接包裹的图与编辑视图里的图不进入查看器。
